...
export function provinceHorizon(epi, coverage, maxWeeks = 26) {
  ...
  const withCases = new Set(records.filter(r => r.date === epi.date && valueAt(r.location,epi.date) > 0).map(r => r.location));
  ...
  return { weeks, groups, band: Math.max(1,Math.ceil(max / 3)),

...
import { caseTrendDetails, areaCaseBasis } from './caseTrend.js';
...
export function keyMessage({datasets=[],epi,mining,security,mobility,asOf,override=''}) {
  ...
  const trend=caseTrendDetails({datasets,epi,asOf});
  const paragraphs=[trend.text];
  const sentences=[];
  ...
  const basis=trend.basis.map(item=>({...item,role:'Trend'}));
  const area=areaCaseBasis(epi);
  if(area&&!basis.some(item=>item.sourceId===area.sourceId))basis.push({...area,role:'Area priorities'});
  else if(area&&basis.length)basis[0].role='Trend and area priorities';
  return {text:paragraphs.join('\n\n'),origin:'Summary from loaded data',basis};
}

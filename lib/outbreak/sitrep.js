import { epiWeek, validDate, formatValue } from './data.js';
...
export function risingReports(epi) {
  if (!epi) return 'Area-level case evidence unavailable.';
  const paired = epi.growth.filter(z => Number.isFinite(z.delta));
  const period = `${epi.baseline}–${epi.date}${epi.comparisonDays ? `; ${epi.comparisonDays} days` : ''}`;
  if (!paired.length) return `Seven-day comparison unavailable: no areas have valid paired observations for ${period}. This does not mean there were no increases.`;
  const rising = paired.filter(z => z.delta > 0).slice(0, 3);
  const coverage = `${paired.length}/${epi.zones.length} areas have paired observations`;
  if (!rising.length) return `No positive change among comparable area reports (${period}; ${coverage}). Areas without paired observations cannot be assessed.`;
  return `${rising.map(z => `${z.location}: +${formatValue(z.delta)}`).join('; ')} (${period}; ${coverage}). Review case investigations and surveillance workload.`;
}

// Cumulative intervals share endpoints and use actual reports 6–8 days apart.
// Daily sums still require all seven observations; missing values are never filled.
export function reportingPeriods(dataset, locations, asOf, count = 8) {
  ...
  for (let i = 0; i < count; i++) {
    const interval = dataset.kind === 'cumulative'
      ? comparisonBaseline(valueAt, locations, end, true)
      : {start: shiftDate(end, -6), end, days: 7};
    let value = 0, missing = false;
    for (const location of locations) {
      if (dataset.kind === 'cumulative') {
        const current = valueAt(location, end), previous = valueAt(location, interval.start);
    ...
    periods.unshift({ ...interval, value: missing ? null : value });
    end = dataset.kind === 'cumulative' ? interval.start : shiftDate(end, -7);
  }
  return periods;
}
...
function comparisonRow(dataset, locations, label, asOf, grouped = false) {
  ...
  return { id: `${dataset.id}:${label}`, label, previous: previous ?? null, current: current ?? null,
    delta: Number.isFinite(previous) && Number.isFinite(current) && periods[0].days === periods[1].days ? current - previous : null,
    periods,

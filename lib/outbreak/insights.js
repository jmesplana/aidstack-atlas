...
export function integratedEvidence(epi,mining,security,mobility,hasGeometry) {
  ...
  if(mobility) {
    ...
    const reported=mobility.records.filter(r=>r.value!==null).length;
    facts.push({id:'integrated:mobility',sourceId:'flowminder',source:`Uploaded GeoJSON: ${mobility.id}`,
      basis:[{sourceId:mobility.id,role:'Mobility indicator',label:mobility.label,source:`Uploaded GeoJSON: ${mobility.id}`,
        url:mobility.definition?.documentation||null,period:mobility.date?`${mobility.date} (${mobility.dateBasis||'date basis not supplied'})`:'date not supplied',
        coverage:`${reported}/${mobility.records.length} areas report a value · ${mobility.unit}`}],

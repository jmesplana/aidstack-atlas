...
export function evidenceReadiness(datasets, asOf) {
  return datasets.map(dataset => {
    ...
    const history = (dataset.records || []).filter(row => row.date <= asOf);
    ...
    return {
      ...
      historyStart: historyDates[0], historyEnd: historyDates.at(-1),
      observations: history.filter(row => Number.isFinite(row.value)).length,

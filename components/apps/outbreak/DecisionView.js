...
export default function DecisionView({ screenRef, title, epi, geometry, boundaryLevel, message, coverage, horizon, alerts, alertDays, asOf, location, province, focus, callout, onSelect, onProvince, onClear, onClose, onAnalysis, freshness }) {
  ...
  useEffect(()=>{
    ...
    const keyboard=e=>{
      ...
      if(e.key!=='Tab')return;
      const nodes=[...screenRef.current.querySelectorAll('button, a[href], [tabindex="0"]')].filter(n=>!n.disabled&&n.getClientRects().length);
  ...
  return createPortal(<dialog ref={dialogRef} aria-label="Decision dashboard" className={styles.decisionDialog} onCancel={e=>{e.preventDefault();closeRef.current();}}><div ref={screenRef} className={`${styles.app} ${styles.decisionScreen}`}>
    ...
    <div className={styles.decisionMain}>
      <section className={`${styles.decisionPanel} ${styles.decisionMap}`} aria-label="Decision map"><div className={styles.decisionPanelHeading}><h2>Where cases are reported</h2>{(province||location)&&<button onClick={onClear}>Show all areas</button>}</div><OutbreakMap presentation geometry={geometry} rows={epi?.zones||[]} level={epi?.dataset.level} kind="cumulative" unit="confirmed cases" boundaryLevel={boundaryLevel} selected={location} onSelect={onSelect} label="Reported cumulative cases" asOf={asOf} source={epi?.dataset.source||epi?.dataset.url} focusNames={focus} highlightNames={alerts.map(a=>a.location)} groupLabels callout={callout}/></section>
      <section className={styles.decisionPanel} aria-label="Decision province coverage"><h2>Province coverage</h2><div className={styles.decisionScroll}>{coverage?.rows.length?<table><thead><tr><th>Province</th><th>Cases</th><th>Affected / total HZ</th><th>Affected</th></tr></thead><tbody>{coverage.rows.map(r=><tr key={r.province} aria-selected={province===r.province}><td><button onClick={()=>onProvince(r.province)}>{r.province}</button></td><td>{number(r.cases)}{r.missing>0&&r.reported>0&&<small>Partial</small>}</td><td>{r.reported?r.affected:'?'} / {r.total}</td><td>{r.reported?`${r.percent.toFixed(1)}%`:'Unknown'}</td></tr>)}</tbody></table>:<p>Province coverage unavailable.</p>}</div><p className={styles.decisionNote}>Affected = reported cumulative cases. Totals use loaded boundaries; province completeness is unverified. {missing>0?`${missing} health zones have no report at this date.`:'Missing data remains unknown.'}</p></section>
    </div>
    <section className={`${styles.decisionPanel} ${styles.decisionTrends}`} aria-label="Decision health-zone trends"><div className={styles.decisionPanelHeading}><h2>Health-zone trends{province?` · ${province}`:''}{location?` / ${location}`:''}</h2><span>Weekly reported changes · darker = larger · purple = revisions · × = missing · * = partial week</span></div><div className={styles.decisionScroll}><ProvinceHorizon model={horizon} compact/></div></section>

...
import { mobilitySummary } from '../../../lib/outbreak/mobility';
...
export default function Outbreak({ storage, districts=[], facilities=[], acledData=[], disasters=[], onOpenWorkspace, leaveGuard }) {
  const briefElement=useRef(null),explorerElement=useRef(null),mobilityElement=useRef(null),refreshGeneration=useRef(0),autoConnection=useRef('');
  ...
  // Load the snapshot to diff against for "since last brief": the chosen one, else the most recent saved snapshot other than the open record.
  const effectiveCompareId=compareId||saved.filter(s=>s.id!==record?.id).sort((a,b)=>String(b.metadata?.name||'').localeCompare(String(a.metadata?.name||'')))[0]?.id||'';
  useEffect(()=>{let live=true;if(!effectiveCompareId){setCompareSnapshot(null);return;}storage.loadPlan(effectiveCompareId).then(s=>{if(live)setCompareSnapshot(s);}).catch(()=>{if(live)setCompareSnapshot(null);});return()=>{live=false;};},[effectiveCompareId,storage]);
  useEffect(()=>{
  ...
  const mobility=useMemo(()=>mobilitySummary(routeData,epi,asOf),[routeData,epi,asOf]);
  ...
  function briefingText() {
    ...
    return [`# ${name}`,`Status: ${reviewed?'Reviewed by user':'DRAFT — requires coordinator review'}`,`Reporting cut-off: ${asOf}. Generated: ${new Date().toISOString()}.`,
      ...
      ...(openingMessage.basis||[]).map(item=>`${item.role}: ${item.label} | Source: ${item.source}${item.url&&item.url!==item.source?` (${item.url})`:''} | Period: ${item.period} | Coverage: ${item.coverage}`),
      ...(since&&since.lines.length?[`## Since last brief${since.priorAsOf?` (${since.priorName||'snapshot'}, ${since.priorAsOf})`:''}`,...since.lines.map(l=>`- ${l.label}: ${l.value}${typeof l.delta==='number'?` (${l.delta>=0?'+':''}${formatValue(l.delta)}${l.kind==='cumulative'&&l.delta<0?'; revision':''})`:''}${l.since?` — ${l.since}`:''}`)]:[]),
  ...
  return <section className={styles.app} aria-label="Outbreak response"><fieldset disabled={!!busy} className={styles.fieldset}>
    ...
    {tab==='Situation'&&<>
      ...
      <div ref={mobilityElement}>
        <Routes overlays={movementOverlays} showFocus={false} direction={routeDirection} onDirection={d=>{change();setRouteDirection(d);}} limit={routeLimit} onLimit={v=>{change();setRouteLimit(v);}} data={routeData} onLoad={fetchRoutes} loading={routeLoading} error={routeError} epi={epi} security={security} geometry={geography.data} boundaryLevel={boundaryLevel} asOf={asOf} selected={selectedLocation} onSelect={chooseArea} layerCount={mobilityLayers.length} onLayers={()=>requestAnimationFrame(()=>document.getElementById('outbreak-mobility-indicators')?.scrollIntoView({behavior:'smooth',block:'start'}))}/>
        {mobilityLayers.length>0&&<div id="outbreak-mobility-indicators"><MobilityPanel overlays={movementOverlays} layers={mobilityLayers} selected={selectedMobility} direction={direction} onDirection={d=>{change();setMovementDirection(d);setMovementField('');}} onLayer={id=>{change();setMovementField(id);}} geometry={geography.data} boundaryLevel={boundaryLevel} asOf={asOf} onSelect={chooseArea}/></div>}
      </div>
      ...
      <p>Choose an area to inspect its indicator trend, observations and map. Movement connections are above.</p>
      ...
      <details><summary>Additional comparisons</summary>
    ...
    {tab==='Briefing'&&<div className={styles.brief} ref={briefElement}>
      ...
      {since&&since.lines.length>0&&(()=>{
        const renderLine=(l,i)=><li key={i}><strong>{l.label}</strong> {l.value} {l.delta!==undefined&&l.delta!==null&&typeof l.delta==='number'?<Delta delta={l.delta} kind={l.kind} neutral={l.neutral}/>:null} {l.since?<small style={{display:'inline'}}>· {l.since}</small>:null}</li>;
        const headline=since.lines.filter(l=>l.group!=='response'),response=since.lines.filter(l=>l.group==='response');
        // Response indicators stay separate and are never dropped; they are only collapsed on
        // screen when numerous. Print and export always show every line.
        const collapse=response.length>6;
        return <div className={styles.panel}><h3>Since last brief{since.priorAsOf?` (${since.priorName||'snapshot'}, ${since.priorAsOf})`:''}</h3>
          <ul className={styles.sinceList}>{headline.map(renderLine)}{!collapse&&response.map((l,i)=>renderLine(l,`r${i}`))}</ul>
          {collapse&&<><details className={styles.noPrint}><summary>{response.length} response indicators compared separately</summary><ul className={styles.sinceList}>{response.map((l,i)=>renderLine(l,`r${i}`))}</ul></details><ul className={`${styles.sinceList} ${styles.printOnly}`}>{response.map((l,i)=>renderLine(l,`p${i}`))}</ul></>}
          <small className={styles.noPrint}>Compared against a saved snapshot. Changes may include revisions and differ from onset-based incidence.</small></div>;
      })()}

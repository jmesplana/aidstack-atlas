...
export default function Outbreak({ storage, districts=[], facilities=[], acledData=[], disasters=[], onOpenWorkspace, leaveGuard }) {
  ...
  const availableDatasets=useMemo(()=>[...datasets,...embedded.filter(d=>!datasets.some(source=>(source.id===d.id||source.metricId&&source.metricId===d.metricId)&&source.status==='ready'))].map(d=>reconcileLocations(d,geography.data,boundaryLevel)),[datasets,embedded,geography,boundaryLevel]);
  ...
  useEffect(()=>{setLocation(current=>selected?.locationMatching?.locations.find(r=>r.source===current)?.location||current);},[selected]);
  ...
  return <section ref={appElement} className={styles.app} aria-label="Outbreak response"><fieldset disabled={!!busy||compareLoading} className={styles.fieldset}>
    ...
    <div hidden={tab!=='Data'}>
      ...
      <div className={styles.panel}><GeoImport layers={geoLayers} level={boundaryLevel} onImport={d=>{change();setDatasets(old=>[...old,d]);setSelectedId(d.id);if(d.purpose==='cases')setEpiSource(d.id);setNotice('GeoJSON indicator mapped. Insights are available in Situation.');}}/></div>
        ...
        sources={<>
      {availableDatasets.filter(d=>d.locationMatching).map(d=><section key={d.id} className={styles.panel}><h3>{d.label}</h3><LocationMatching dataset={d}/></section>)}
